...
Save(n) {
    ...
    SplitPath FullName,, g_SaveDir, FileExt
    If (FileExt = "ahk") {
        TabEx.SetIcon(n, 2)
    } Else If (g_GuiTab != n) {
        TabEx.SetIcon(n, 1)
...
Compile() {
    ...
    ; ScriptFile := Sci[n].FullName
    ; ExeFile := Dir . "\" . NameNoExt . ".exe"
    Try {
        Run %Ahk2ExePath%,,, PID ; /in "%ScriptFile%" /out "%ExeFile%
    }
    ...
    If (Sci[n].FullName && !Sci[n].GetModify()) {
        ...
        ExeFile := "C:\Program Files\AutoHotkey\Scripts\bin\" . NameNoExt . ".exe" ; Dir
        ...
        ; }
        ; MsgBox,,test,ScriptFile: %ScriptFile%`nExeFile: %ExeFile%
