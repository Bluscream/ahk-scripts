...
VA_GetDevices(count := 15) {
    lst := {}
    Loop, % count {
        dev := VA_GetDevice(A_Index)
        name := VA_GetDeviceName(dev)
        lst[name] := dev
    }
    return lst
}

VA_PlaybackDeviceState(device, bool := False){
	; CPolicyConfigClient
	...
	if (PolConfig := ComObjCreate(CLSID, IID)) {
        if (VA_IMMDevice_GetId(defPlaybackDevice, id) == 0) { ; get the device id
            DllCall(NumGet(NumGet(PolConfig+0)+14*A_PtrSize), "Ptr", PolConfig, "WStr", id, "Int", bool)
        }
